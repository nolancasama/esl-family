...
const Photo = (() => {
  ...
  function drawCover(ctx, img, x, y, w, h, biasY, zoom = 1, biasX) {
    if (!img) {
      ctx.fillStyle = '#fff';
    ...
    const dx = x - (sw - w) * (biasX ?? 0.5);
    const dy = y - (sh - h) * (biasY ?? 0.5);
    ctx.drawImage(img, dx, dy, sw, sh);
  }

  /* ── One framed portrait, with nameplate below ────────────────────── */
  function drawFrame(ctx, x, y, w, h, portraitImg, frameInfo, frameImg, label, biasY, zoom = 1, biasX) {
    ...
    drawCover(ctx, portraitImg, holeX + padX, holeY + padY, holeW - padX * 2, holeH - padY * 2, biasY, zoom, biasX);
    ctx.restore();
  ...
  function drawCard(ctx, card, scale) {
    ...
    drawFrame(ctx, card.x, card.y, card.w, card.h, card.img, card.frameInfo, card.frameImg, card.label, card.biasY, card.zoom, card.biasX);
    ctx.restore();
  }
  ...
  async function compose(canvasEl, assignments, selfieDataURL, playerName) {
    ...
    front.positions.forEach((pos, i) => {
      ...
      cards.push({
        ...
        biasX: k !== 'me' && char.id === 'gf52' ? 0.28 : 0.5,
      });
    });
